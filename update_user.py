...
mtitle=Label(mangefrm,text="Manage Patient",font=("Times New Roman",20,"bold"),fg="darkgreen",bg="light blue")
...
name_box.grid(row=1,column=1,pady=10,padx=20,sticky="w")

address=Label(mangefrm,text="Address",font=("Times New Roman",15,"bold"),bg="light blue")
address.grid(row=2,column=0,pady=10,padx=20,sticky="w")
address_box=Entry(mangefrm,font=("Times New Roman",15,"bold"),bd=5,relief=GROOVE)
address_box.grid(row=2,column=1,pady=10,padx=20,sticky="w")
...
contact_box.grid(row=3,column=1,pady=10,padx=20,sticky="w")
...
email_box.grid(row=4,column=1,pady=10,padx=20,sticky="w")

password=Label(mangefrm,text="Password",font=("Times New Roman",15,"bold"),bg="light blue")
password.grid(row=5,column=0,pady=10,padx=20,sticky="w")
password_box=Entry(mangefrm,font=("Times New Roman",15,"bold"),bd=5,relief=GROOVE,show='*')
password_box.grid(row=5,column=1,pady=10,padx=20,sticky="w")
...
def fectdata():
    ...
    mycursor.execute("select name,address,contact,email,password from patient")
...
def update():
    ...
    try:
        ...
        sql = "update patient set name=%s,address=%s,contact=%s,email=%s,password=%s where name=%s"

        val = (nm, ad, co, em, pwd, nm)
...
def delete1():
    ...
    try:
        ...
        sql = "delete from patient where email=%s"
...
def fectdata1():
    ...
    mycursor.execute("select name,address,contact,email,password from patient where "+str(ser1)+" LIKE '%"+str(lsearch1)+"%'")
...
medtab=ttk.Treeview(tabfrm,columns=("Name","Address","Contact","Email","Password"),xscrollcommand=scrollx.set,yscrollcommand=scrolly.set)
